...
function initializeAutoTrimps() {
    ...
    loadPageVariables();
    javascript: with(document)(head.appendChild(createElement('script')).src = 'https://reinx.github.io/AutoTrimps/NewUI.js')._;
...
function autoMap() {
	...
	needToVoid = getPageSetting('VoidMaps') > 0 && game.global.totalVoidMaps > 0 && game.global.lastClearedCell >= 97 && ((game.global.world == getPageSetting('VoidMaps') && !getPageSetting('RunNewVoids')) || (game.global.world >= getPageSetting('VoidMaps') && getPageSetting('RunNewVoids'))) && (game.global.challengeActive != 'Lead' || game.global.lastClearedCell > 95);
    if (game.global.mapsUnlocked) {
       ...
       var enemyhp = (getEnemyMaxHealth(game.global.world, 99));
		...
		case "Helium Per Hour":
			if(game.global.world > lastZone) {
				...
				var timeThisPortal = new Date().getTime() - game.global.portalTime;
	    			...
	    			if(myHelium < lastHelium && !game.global.challengeActive && game.global.world > getPageSetting('VoidMaps')) {
	    				pushData();
...
function manageGenes() {
    ...
    if (targetBreed > getBreedTime() && !game.jobs.Geneticist.locked && game.resources.trimps.soldiers > 0) {
    ...
    else if (targetBreed < getBreedTime() && !game.jobs.Geneticist.locked) {
